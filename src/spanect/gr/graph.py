...
'''
@File    :   graph.py
@Time    :   2025/01/07 14:02:17
@Author  :   zzrs123@DuGaoyuan 
@Contact :   zzrs123.gaoyuan@gmail.com
'''
...
class graph():
    ...
    def mx2SparseTensor(self, mx):
        ...
        col = torch.from_numpy(mx.col).to(torch.long)
        values = torch.from_numpy(mx.data)
        adj = torch.sparse_coo_tensor(torch.stack([row, col]), values, size=mx.shape)
        return adj
    ...
    def main(self):
        ...
        if adj_norm.layout == torch.sparse_coo:
            adj_norm = adj_norm.to_dense()  # 转换为密集张量
        edge_index = adj_norm.nonzero().t()  # 转换为 [2, E] 格式
        # print("edgeindex")
        graph_dict = {
                     "adj_norm": adj_norm,
                     "adj_label": adj_label,
